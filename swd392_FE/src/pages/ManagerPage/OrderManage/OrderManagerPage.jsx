...
function OrderManagerPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const orderData = response.data.items?.$values || [];
      setOrders(orderData);
      setPagination((prev) => ({
        ...prev,
  ...
  const fetchProductDetails = async (detail) => {
    ...
    try {
      ...
      const cacheKey = detail.packageId
        ? `package_${detail.packageId}`
        : detail.blindBoxId
        ? `blindbox_${detail.blindBoxId}`
        : null;
      ...
      switch (newStatus.toLowerCase()) {
        ...
        case "paid":
          if (currentStatus !== "pending") {
            toast.error(
              "Chỉ đơn hàng ở trạng thái Chờ xử lý mới có thể chuyển sang Đã thanh toán"
            );
            return;
          }
          break;
        ...
        case "shipping":
          if (currentStatus !== "paid") {
            toast.error(
              "Chỉ những đơn hàng đã thanh toán mới có thể chuyển sang trạng thái vận chuyển"
            );
            return;
          }

          await axios.post(
            `${BASE_URL}/orders/confirm-order?orderId=${orderId}`,
            {},
            {
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        case "completed":
          if (currentStatus !== "shipping") {
            toast.error(
              "Chỉ đơn hàng ở trạng thái Đang vận chuyển mới có thể chuyển sang Hoàn thành"
            );
            return;
          }
          const isStorePickup =
            selectedOrder.address &&
            selectedOrder.address.toLowerCase().includes("nhận tại cửa hàng");
          if (!isStorePickup) {
            toast.error(
              "Chỉ đơn hàng có địa chỉ 'Nhận tại cửa hàng' mới có thể chuyển sang Hoàn thành"
            );
            return;
          }
          await axios.post(
        ...
        case "canceled":
          ...
          await axios.post(
            `${BASE_URL}/orders/cancel-order?orderId=${orderId}`,
            {},
            {
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                {selectedOrder.statuses?.$values?.length > 0 ? (
                      ...
                      .map((statusItem, idx) => (
                        <div key={idx} className="flex items-start">
                          <div className="relative">
                            ...
                            ></div>
                            {idx <
                              selectedOrder.statuses.$values.length - 1 && (
                  ...
                  <p className="text-gray-500">
                    Không có thông tin lịch sử trạng thái
                  </p>
                )}
              </div>
            </div>
                  ...
                  {selectedOrder.details?.$values?.map((detail, idx) => (
                    <tr key={idx}>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center">
                          {detail.packageId &&
                            productDetails[`package_${detail.packageId}`]
                              ?.images?.$values?.[0]?.url && (
                          ...
                          {detail.blindBoxId &&
                            productDetails[`blindbox_${detail.blindBoxId}`]
                              ?.imageUrls?.$values?.[0] && (
                              <div className="flex-shrink-0 h-10 w-10 mr-3">
                                <img
                                  className="h-10 w-10 rounded-md object-cover"
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={() =>
                    updateOrderStatus(selectedOrder.orderId, "Paid")
                  }
                  disabled={
                    getCurrentStatus(selectedOrder)?.toLowerCase() !== "pending"
                  }
                  className={`px-4 py-2 text-sm rounded-lg border ${
                    getCurrentStatus(selectedOrder)?.toLowerCase() === "paid"
                      ...
                      : getCurrentStatus(selectedOrder)?.toLowerCase() !==
                        "pending"
                ...
                <button
                  onClick={() =>
                    updateOrderStatus(selectedOrder.orderId, "Shipping")
                  }
                  disabled={
                    getCurrentStatus(selectedOrder)?.toLowerCase() !== "paid"
                  }
                ...
                <button
                  onClick={() =>
                    updateOrderStatus(selectedOrder.orderId, "Canceled")
                  }
                  disabled={["shipping", "completed"].includes(
                    getCurrentStatus(selectedOrder)?.toLowerCase()
                  )}
                  className={`px-4 py-2 text-sm rounded-lg border ${
                    getCurrentStatus(selectedOrder)?.toLowerCase() ===
